...
const openai = new OpenAI({
    ...
    return (
        <>
            ...
            <button onClick={handleChatCompletion} className={styles.btnPrompt} disabled={loading}>Enviar</button>
            {loading && <div className={styles.ProgressBar}></div>}
            <div className={styles.respostaGPT}>
                {aiResponse.split(/(?<![0-9])[.;:](?![0-9])|.(?=-)/).map((paragraph, index) => {
